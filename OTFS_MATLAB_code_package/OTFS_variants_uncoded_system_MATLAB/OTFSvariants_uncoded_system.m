...
for iesn0 = 1:length(SNR_dB)
    for ifram = 1:N_fram
        ...
        r=Gen_channel_output_OTFSvariants(N,M,L_set,gs,s,sigma_2(iesn0),variant,length_CP);
        ...
        %% MRC delay-time detection in [R1,R2]
        ...
        %% See Section 4.5 and Section 6.5.6 in [R1] for OTFS variants MRC detection
        if(strcmp(variant,'RZP'))
            [est_info_bits_MRC,det_iters_MRC,data_MRC] = MRC_delay_time_detector_RZP(N,M,M_mod,sigma_2(iesn0),data_grid,Y_tilda,H_t_f,n_ite_MRC,omega,r,Fn,decision,L_set,nu_ml_tilda,init_estimate);
        elseif(strcmp(variant,'RCP'))
            [est_info_bits_MRC,det_iters_MRC,data_MRC] = MRC_delay_time_detector_RCP(N,M,M_mod,sigma_2(iesn0),data_grid,Y_tilda,H_t_f,n_ite_MRC,omega,r,Fn,decision,L_set,nu_ml_tilda,init_estimate);
        elseif(strcmp(variant,'CP'))
            [est_info_bits_MRC,det_iters_MRC,data_MRC] = MRC_delay_time_detector_CP(N,M,M_mod,sigma_2(iesn0),data_grid,Y_tilda,H_t_f,n_ite_MRC,omega,r,Fn,decision,L_set,nu_ml_tilda,init_estimate);
...
legend('OTFS MRC detection (Chapter 6, Algorithm 3 in [R1])')
grid on
xlabel('SNR(dB)')
ylabel('BER')
