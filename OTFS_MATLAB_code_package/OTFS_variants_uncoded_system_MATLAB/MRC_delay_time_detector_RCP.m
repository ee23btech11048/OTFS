...
function [est_bits,ite,x_data] = MRC_delay_time_detector_RCP(N,M,M_mod,no,data_grid,Y_tilda,H_tf,n_ite_MRC,omega,r,Fn,decision,L_set,nu_ml_tilda,init_estimate)
%% Initial assignments
...
for m=1:M
    for l=L_set
        if(m+l-1<=M)
            ...
            d_m_tilda(:,m)=d_m_tilda(:,m)+circshift(abs(nu_ml_tilda(:,mod(m+(l-1)-1,M)+1,l).^2),-1);        % See Section 4.5.2 in Chapter 4, [R1]
        end
    end
end
for m=1:M
    for l=L_set
        if(m>=l)
            ...
            delta_y_m_tilda(:,m)=delta_y_m_tilda(:,m)-nu_ml_tilda(:,m,l).*circshift(x_m_tilda(:,mod(m-(l-1)-1,M)+1),1); % See Section 4.5.2 in Chapter 4, [R1]
        end
    end
end
    ...
    for m=1:M
        for l=L_set
            if(m+(l-1)<=M)
                ...
                delta_g_m_tilda(:,m)=delta_g_m_tilda(:,m)+circshift(conj(nu_ml_tilda(:,mod(m+(l-1)-1,M)+1,l)).*delta_y_m_tilda(:,mod(m+(l-1)-1,M)+1),-1);  % See Section 4.5.2 in Chapter 4, [R1]
            end
        end
        ...
        for l=L_set                                                                                                                  % Line 12 of Algorithm 3 in Chapter 12, in [R1]
            if(m+(l-1)<=M)
                ...
                delta_y_m_tilda(:,mod(m+(l-1)-1,M)+1)=delta_y_m_tilda(:,mod(m+(l-1)-1,M)+1)-nu_ml_tilda(:,mod(m+(l-1)-1,M)+1,l).*circshift((x_m_tilda(:,m)-x_m_tilda_old(:,m)),1); % See Section 4.5.2 in Chapter 4, [R1]
